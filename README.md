...
| <a name="input_association_targets"></a> [association\_targets](#input\_association\_targets) | Optional list of target filters for the SSM association. Supported key formats: `"InstanceIds"` (use `["*"]` for all), `"tag:<TagKey>"` (target by EC2 tag — **max 1 entry for Automation documents** like CrowdStrike-FalconSensorDeploy; use a ResourceGroup to AND multiple tags), `"ResourceGroup"` (target a named AWS Resource Groups group). When not set, all managed instances are targeted. | <pre>list(object({<br>    key    = string<br>    values = list(string)<br>  }))</pre> | `null` | no |
