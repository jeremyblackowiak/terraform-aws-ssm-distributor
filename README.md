...
| <a name="input_association_targets"></a> [association\_targets](#input\_association\_targets) | Optional target for the SSM association. Specify one of: `key = "InstanceIds"` with `values = ["*"]` for all instances; `key = "tag:<TagKey>"` to target by EC2 tag (Automation docs support only one tag key — use ResourceGroup to AND multiple tags); `key = "ResourceGroup"` with `values = ["<group-name>"]` to target an AWS Resource Group. When not set, all managed instances are targeted. | <pre>object({<br>    key    = string<br>    values = list(string)<br>  })</pre> | `null` | no |
